...
public abstract class CriAtomExVoicePool : CriDisposable
{
	...
	{
		/* 機種共通のボイスプールID */
#if UNITY_EDITOR || UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX || UNITY_ANDROID || UNITY_IOS || UNITY_TVOS || UNITY_PS3 || UNITY_PS4 || UNITY_PS5 || UNITY_WINRT || UNITY_XBOXONE || UNITY_GAMECORE_XBOXONE || UNITY_GAMECORE_SCARLETT || UNITY_WEBGL || UNITY_SWITCH || UNITY_STADIA || UNITY_STANDALONE_LINUX || UNITY_OPENHARMONY
		...
		/* 機種固有のボイスプールID */
#if UNITY_EDITOR || UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX || UNITY_IOS || UNITY_TVOS || UNITY_PS3 || UNITY_WINRT || UNITY_XBOXONE || UNITY_GAMECORE_XBOXONE || UNITY_GAMECORE_SCARLETT || UNITY_WEBGL || UNITY_SWITCH || UNITY_STADIA || UNITY_STANDALONE_LINUX || UNITY_OPENHARMONY
#elif UNITY_ANDROID
...
#error unsupported platform
	...
	{
		UsedVoicesInfo info;
		CRIWAREF606FF1B((int)voicePoolId, out info.numUsedVoices, out info.numPoolVoices);
		return info;
	}
	...
	private static extern void CRIWAREF606FF1B(int voice_pool_id, out int num_used_voices, out int num_pool_voices);
	...
	#endif
	#else
	private static void CRIWAREF606FF1B(int voice_pool_id, out int num_used_voices, out int num_pool_voices)
...
public class CriAtomExStandardVoicePool: CriAtomExVoicePool
{
	...
	private static void CRIWAREEBBAD341(ref Config config) { }
	#endif
	#endregion
}
...
public class CriAtomExWaveVoicePool: CriAtomExVoicePool
{
	...
	private static void CRIWARE7D9C1C2B(ref Config config) { }
	#endif
	#endregion
}

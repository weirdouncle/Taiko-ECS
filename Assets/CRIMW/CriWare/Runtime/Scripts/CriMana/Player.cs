 ...
 */
	public class Player : CriDisposable
	{
		...
		{
			get {
				return NativeMethods.GetNumberOfEntry(playerId);
			}
		}
		...
		public Player(bool advanced_audio_mode, bool ambisonics_mode, uint max_path_length) {
			...
			if (advanced_audio_mode || max_path_length > 0) {
				playerId = NativeMethods.CreateWithParameters(advanced_audio_mode, max_path_length);
				if (advanced_audio_mode) {
					/* AtomExPlayer 作成 */
					_atomExPlayer = new CriAtomExPlayer(NativeMethods.GetAtomExPlayerByTrackId(playerId, 0));
					_subAtomExPlayer = new CriAtomExPlayer(NativeMethods.GetAtomExPlayerByTrackId(playerId, 1));
					_extraAtomExPlayer = new CriAtomExPlayer(NativeMethods.GetAtomExPlayerByTrackId(playerId, 2));
					if (ambisonics_mode) {
		...
		{
			NativeMethods.Pause(playerId, (sw) ? 1 : 0);
		...
		{
			return NativeMethods.IsPaused(playerId);
		}
		...
		public bool SetFile(CriFsBinder binder, string moviePath, SetMode setMode = SetMode.New)
		{
			...
			if (setMode == SetMode.New) {
				NativeMethods.SetFile(playerId, binderPtr, moviePath);
				return true;
			} else {
				return NativeMethods.EntryFile(
		...
		{
			if (setMode == SetMode.New) {
				NativeMethods.SetData(playerId, data, dataSize);
				return true;
			} else {
				return NativeMethods.EntryData(
		...
		*/
		...
		public bool SetContentId(CriFsBinder binder, int contentId, SetMode setMode = SetMode.New)
		{
			...
			if (setMode == SetMode.New) {
				NativeMethods.SetContentId(playerId, binder.nativeHandle, contentId);
				return true;
			}
			else {
				return NativeMethods.EntryContentId(
		...
		{
			if (setMode == SetMode.New) {
				NativeMethods.SetFileRange(playerId, filePath, offset, range);
				return true;
			}
			else {
				return NativeMethods.EntryFileRange(
		...
		{
			NativeMethods.Loop(playerId, sw ? 1 : 0);
		}
		...
		{
			NativeMethods.SetAudioBaseConcatenation(playerId, enabled);
		}
		...
		public void SetMasterTimerType(TimerType timerType)
		{
			...
			NativeMethods.SetMasterTimerType(playerId, timerType);
		}
		...
		{
			NativeMethods.SetSeekPosition(playerId, frameNumber);
		}
		...
		{
			NativeMethods.SetMovieEventSyncMode(playerId, mode);
		}
		...
		{
			NativeMethods.SetSpeed(playerId, speed);
		}
		...
		{
			NativeMethods.SetMaxPictureDataSize(playerId, maxDataSize);
		}
		...
		{
			NativeMethods.SetBufferingTime(playerId, sec);
		}
		...
		{
			NativeMethods.SetMinBufferSize(playerId, min_buffer_size);
		}
		...
		{
			NativeMethods.SetAudioTrack(playerId, track);
		}
		...
		public void SetAudioTrack(AudioTrack track)
		{
			if (track == AudioTrack.Off) {
				NativeMethods.SetAudioTrack(playerId, -1);
			} else if (track == AudioTrack.Auto) {
				NativeMethods.SetAudioTrack(playerId, 100);
			}
		}
		...
		{
			NativeMethods.SetSubAudioTrack(playerId, track);
		}
		...
		{
			if (track == AudioTrack.Off) {
				NativeMethods.SetSubAudioTrack(playerId, -1);
			} else if (track == AudioTrack.Auto) {
				NativeMethods.SetSubAudioTrack(playerId, 100);
			}
		}
		...
		{
			NativeMethods.SetExtraAudioTrack(playerId, track);
		}
		...
		{
			if (track == AudioTrack.Off) {
				NativeMethods.SetExtraAudioTrack(playerId, -1);
			} else if (track == AudioTrack.Auto) {
				NativeMethods.SetExtraAudioTrack(playerId, 100);
			}
		}
		...
		{
			NativeMethods.SetVolume(playerId, volume);
		}
		...
		{
			return NativeMethods.GetVolume(playerId);
		}
		...
		{
			NativeMethods.SetSubAudioVolume(playerId, volume);
		}
		...
		{
			return NativeMethods.GetSubAudioVolume(playerId);
		}
		...
		{
			NativeMethods.SetExtraAudioVolume(playerId, volume);
		}
		...
		{
			return NativeMethods.GetExtraAudioVolume(playerId);
		}
		...
		{
			NativeMethods.SetBusSendLevelByName(playerId, bus_name, level);
		}
		...
		{
			NativeMethods.SetSubAudioBusSendLevelByName(playerId, bus_name, volume);
		}
		...
		{
			NativeMethods.SetExtraAudioBusSendLevelByName(playerId, bus_name, volume);
		}
		...
		public void SetSubtitleChannel(int channel)
		{
			...
			if (enableSubtitle) {
				NativeMethods.SetSubtitleCallback(playerId, SubtitleCallbackFromNative);
				...
				NativeMethods.SetSubtitleCallback(playerId, null);
			}
			NativeMethods.SetSubtitleChannel(playerId, channel);
		}
		...
		public long GetTime()
		{
			return NativeMethods.GetTime(this.playerId);
		}
		...
		public int GetDisplayedFrameNo()
		{
			...
			return NativeMethods.GetDisplayedFrameNo(this.playerId);
		}
		...
		{
			NativeMethods.SetAsrRackId(this.playerId, asrRackId);
		}
		...
		{
			NativeMethods.SetTimeStretchQuality(this.playerId, quality);
		}
...
/* @cond notpublic */
		...
		{
			NativeMethods.SetDecryptionKey(this.playerId, key);
		}
/* @endcond */
		...
		public void UpdateWithUserTime(ulong timeCount, ulong timeUnit)
		{
			...
			NativeMethods.SetUserTime(this.playerId, timeCount, timeUnit);
			InternalUpdate();
		}
		...
		public void SetManualTimerUnit(ulong timeUnitN, ulong timeUnitD)
		{
			...
			NativeMethods.SetManualTimerUnit(this.playerId, timeUnitN, timeUnitD);
		}
		...
		public void UpdateWithManualTimeAdvanced()
		{
			...
			NativeMethods.AdvanceManualTimer(this.playerId);
			InternalUpdate();
		}
		...
		{
			NativeMethods.SyncMasterTimer(this.playerId);
		}
	...
	#else
			UnityEngine.GL.IssuePluginEvent(NativeMethods.GetRenderEventFunc(), eventID);
	#endif
#endif
		}
		...
		private void Dispose(bool disposing)
		{
			...
			if (playerId != InvalidPlayerId) {
				...
				NativeMethods.Destroy(playerId);
		...
		private void InternalUpdate()
		{
			...
			NativeMethods.SyncUpdate(playerId);
			...
			{
				NativeMethods.GetMovieInfo(playerId, _movieInfo);
		...
		private void PrepareNativePlayer()
		{
			if (cuePointCallback != null) {
				NativeMethods.SetCuePointCallback(playerId, CuePointCallbackFromNative);
			}
			NativeMethods.Prepare(playerId);
		}
		...
		private void UpdateNativePlayer()
		{
			...
			_nativeStatus = (Status)NativeMethods.Update(playerId);
		...
		internal delegate void CuePointCallbackFromNativeDelegate(System.IntPtr ptr1, System.IntPtr ptr2, [In] ref EventPoint eventPoint);
		...
		[UnmanagedFunctionPointer(CallingConvention.Cdecl)]
		internal delegate void SubtitleCallbackFromNativeDelegate(System.IntPtr ptr1, System.IntPtr ptr2);
		...
		private static void SubtitleCallbackFromNative(System.IntPtr ptr1, System.IntPtr ptr2)
		{
			if (updatingPlayer.OnSubtitleChanged != null) {
				updatingPlayer.subtitleSize = NativeMethods.GetSubtitleOnTime(updatingPlayer.playerId, updatingPlayer.subtitleBuffer, updatingPlayer.subtitleBufferSize);
        ...
        {
            internal static System.Int32 GetNumberOfEntry(int playerId)
            {
				using (new CriDurationMeasurementScope(nameof(CRIWARE1354FEA7)))
                {
					return CRIWARE1354FEA7(playerId);
                }
            }

			internal static int Create()
			{ 
				using (new CriDurationMeasurementScope(nameof(CRIWAREE9FC813E)))
                {
					return CRIWAREE9FC813E();
				}
			}

			internal static int CreateWithParameters(bool useAtomExPlayer, uint maxPathLength)
            {
				using (new CriDurationMeasurementScope(nameof(CRIWARE94E9893B)))
                {
					return CRIWARE94E9893B(useAtomExPlayer, maxPathLength);
                };
            }

			internal static IntPtr GetAtomExPlayerByTrackId(int player_id, uint track_id)
		    {
                using (new CriDurationMeasurementScope(nameof(CRIWARE4E36FD70)))
                {
					return CRIWARE4E36FD70(player_id, track_id);
                }
            }

			internal static void Stop(int player_id)
			{
                using (new CriDurationMeasurementScope(nameof(CRIWAREFC4BD812)))
                {
					CRIWAREFC4BD812(player_id);
                }
			}
			
			internal static void Pause(int player_id, int sw)
			{
                using (new CriDurationMeasurementScope(nameof(CRIWARE2AC4CA7E)))
                {
					CRIWARE2AC4CA7E(player_id, sw);
                }
			}

			internal static bool IsPaused(int player_id)
			{
                using (new CriDurationMeasurementScope(nameof(CRIWARE88ED0C7C)))
                {
					return CRIWARE88ED0C7C(player_id);
                }
			}

			internal static void SetFile(int player_id, System.IntPtr binder, string path)
			{
                using (new CriDurationMeasurementScope(nameof(CRIWARE3FF6C123)))
                {
					CRIWARE3FF6C123(player_id, binder, path);
                }
			}

			internal static bool EntryFile(int player_id, System.IntPtr binder, string path, bool repeat)
			{
                using (new CriDurationMeasurementScope(nameof(CRIWARE340855B4)))
                {
					return CRIWARE340855B4(player_id, binder, path, repeat);
                }
			}

			internal static void SetData(int player_id, IntPtr data, System.Int64 datasize)
			{
                using (new CriDurationMeasurementScope(nameof(CRIWAREBD7E64E1)))
                {
					CRIWAREBD7E64E1(player_id, data, datasize);
                }
			}

			internal static void SetData(int player_id, byte[] data, System.Int64 datasize)
			{
                using (new CriDurationMeasurementScope(nameof(CRIWAREBD7E64E1)))
                {
					CRIWAREBD7E64E1(player_id, data, datasize);
			...
			internal static bool EntryData(int player_id, IntPtr data, System.Int64 datasize, bool repeat)
			{
                using (new CriDurationMeasurementScope(nameof(CRIWARE6587A256)))
                {
					return CRIWARE6587A256(player_id, data, datasize, repeat);
                }
			}

			internal static bool EntryData(int player_id, byte[] data, System.Int64 datasize, bool repeat)
			{
                using (new CriDurationMeasurementScope(nameof(CRIWARE6587A256)))
                {
					return CRIWARE6587A256(player_id, data, datasize, repeat);
			...
			internal static void SetContentId(int player_id, System.IntPtr binder, int content_id)
			{
                using (new CriDurationMeasurementScope(nameof(CRIWARE74F9373A)))
                {
					CRIWARE74F9373A(player_id, binder, content_id);
                }
			}

			internal static bool EntryContentId(int player_id, System.IntPtr binder, int content_id, bool repeat)
			{
                using (new CriDurationMeasurementScope(nameof(CRIWARE89F5F4E8)))
                {
					return CRIWARE89F5F4E8(player_id, binder, content_id, repeat);
                }
			}

			internal static void SetFileRange(int player_id, string path, System.UInt64 offset, System.Int64 range)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWAREAC81C8CF)))
                {
					CRIWAREAC81C8CF(player_id, path, offset, range);
                }
			}

			internal static bool EntryFileRange(int player_id, string path, System.UInt64 offset, System.Int64 range, bool repeat)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE2B9092A9)))
                {
					return CRIWARE2B9092A9(player_id, path, offset, range, repeat);
                }
			}

			internal static void Loop(int player_id, int sw)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE284D752E)))
                {
					CRIWARE284D752E(player_id, sw);
                }
			}

			internal static void SetAudioBaseConcatenation(int player_id, bool flag)
            {
				using (new CriDurationMeasurementScope(nameof(CRIWAREE054EF2C)))
                {
					CRIWAREE054EF2C(player_id, flag);
                }            
			}

			internal static void SetMasterTimerType(int player_id, TimerType timer_type)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE8FCF2B74)))
                {
					CRIWARE8FCF2B74(player_id, timer_type);
                }    
			}

			internal static void SetSeekPosition(int player_id, int seek_frame_no)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWAREAA815027)))
                {
					CRIWAREAA815027(player_id, seek_frame_no);
                }
			}

			internal static void SetMovieEventSyncMode(int player_id, MovieEventSyncMode mode)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARED3E36645)))
                {
					CRIWARED3E36645(player_id, mode);
                }
			}

			internal static void SetSpeed(int player_id, float speed)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE61B8FA6A)))
                {
					CRIWARE61B8FA6A(player_id, speed);
                }
			}

			internal static void SetMaxPictureDataSize(int player_id, System.UInt32 max_data_size)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWAREECCF5E62)))
                {
					CRIWAREECCF5E62(player_id, max_data_size);
                }
			}

			internal static void SetBufferingTime(int player_id, float sec)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWAREB73990A9)))
                {
					CRIWAREB73990A9(player_id, sec);
                }
			}

			internal static void SetMinBufferSize(int player_id, int min_buffer_size)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWAREC4A02CCE)))
                {
					CRIWAREC4A02CCE(player_id, min_buffer_size);
                }
			}

			internal static void SetAudioTrack(int player_id, int track)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE0CA1216B)))
                {
					CRIWARE0CA1216B(player_id, track);
                }
			}

			internal static void SetSubAudioTrack(int player_id, int track)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE9EC619B6)))
                {
					CRIWARE9EC619B6(player_id, track);
                }
			}

			internal static void SetExtraAudioTrack(int player_id, int track)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE894B55A2)))
                {
					CRIWARE894B55A2(player_id, track);
                }
			}

			internal static void SetVolume(int player_id, float vol)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE4F06C1D4)))
                {
					CRIWARE4F06C1D4(player_id, vol);
                }
			}

			internal static float GetVolume(int player_id)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE086C6A97)))
                {
					return CRIWARE086C6A97(player_id);
                }
			}

			internal static void SetSubAudioVolume(int player_id, float vol)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE9B69D97C)))
                {
					CRIWARE9B69D97C(player_id, vol);
                }
			}

			internal static float GetSubAudioVolume(int player_id)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWAREE218A3A4)))
                {
					return CRIWAREE218A3A4(player_id);
                }
			}

			internal static void SetExtraAudioVolume(int player_id, float vol)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE27EEE694)))
                {
					CRIWARE27EEE694(player_id, vol);
                }
			}

			internal static float GetExtraAudioVolume(int player_id)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWAREF299792C)))
                {
					return CRIWAREF299792C(player_id);
                }
			}

			internal static void SetBusSendLevelByName(int player_id, string bus_name, float level)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARED5BBE158)))
                {
					CRIWARED5BBE158(player_id, bus_name, level);
                }
			}

			internal static void SetSubAudioBusSendLevelByName(int player_id, string bus_name, float level)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE068E5808)))
                {
					CRIWARE068E5808(player_id, bus_name, level);
                }
			}

			internal static void SetExtraAudioBusSendLevelByName(int player_id, string bus_name, float level)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE5604C40B)))
                {
					CRIWARE5604C40B(player_id, bus_name, level);
                }
			}

			internal static void SetSubtitleCallback(int player_id, SubtitleCallbackFromNativeDelegate cbfunc)
            {
				using (new CriDurationMeasurementScope(nameof(CRIWARE11EC40C0)))
                {
					CRIWARE11EC40C0(player_id, cbfunc);
                }
            }

			internal static void SetSubtitleChannel(int player_id, int channel)
            {
				using (new CriDurationMeasurementScope(nameof(CRIWARE6F6D87FE)))
                {
					CRIWARE6F6D87FE(player_id, channel);
                }
            }

			internal static long GetTime(int player_id)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE626DF59A)))
                {
					return CRIWARE626DF59A(player_id);
                }
			}

			internal static int GetDisplayedFrameNo(int player_id)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWAREFD3EF64A)))
                {
					return CRIWAREFD3EF64A(player_id);
                }
			}

			internal static void SetAsrRackId(int player_id, int asr_rack_id)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE35F190C7)))
                {
					CRIWARE35F190C7(player_id, asr_rack_id);
                }
			}

			internal static void SetTimeStretchQuality(int player_id, float quality)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWAREF0F20CEB)))
                {
					CRIWAREF0F20CEB(player_id, quality);
                }	
			}

			internal static void SetUserTime(int player_id, ulong user_count, ulong user_unit)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE77CA0811)))
                {
					CRIWARE77CA0811(player_id, user_count, user_unit);
                }	
			}

			internal static void SetManualTimerUnit(int player_id, ulong timer_unit_n, ulong timer_unit_d)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE903CEF6A)))
                {
					CRIWARE903CEF6A(player_id, timer_unit_n, timer_unit_d);
                }	
			}

			internal static void AdvanceManualTimer(int player_id)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE4973B306)))
                {
					CRIWARE4973B306(player_id);
                }	
			}

			internal static void SyncMasterTimer(int player_id)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE041A49E0)))
                {
					CRIWARE041A49E0(player_id);
                }	
			}

			internal static IntPtr GetRenderEventFunc()
			{
				using (new CriDurationMeasurementScope(nameof(criWareUnity_GetRenderEventFunc)))
                {
					return criWareUnity_GetRenderEventFunc();
                }	
			}

			internal static void Destroy(int player_id)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE99111FE9)))
                {
					CRIWARE99111FE9(player_id);
                }	
			}

			internal static void SyncUpdate(int player_id)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE74BAE3A5)))
                {
					CRIWARE74BAE3A5(player_id);
                }	
			}

			internal static void GetMovieInfo(int player_id, [Out] MovieInfo movie_info)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE9A8AC346)))
                {
					CRIWARE9A8AC346(player_id, movie_info);
                }	
			}

			internal static void Start(int player_id)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE439340B3)))
                {
					CRIWARE439340B3(player_id);
                }	
			}

			internal static void Prepare(int player_id)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE7652287A)))
                {
					CRIWARE7652287A(player_id);
                }	
			}

			internal static int Update(int player_id)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWAREC2475753)))
                {
					return CRIWAREC2475753(player_id);
                }	
			}

			internal static void SetCuePointCallback(int player_id, CuePointCallbackFromNativeDelegate cbfunc)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE62E9D3D4)))
                {
					CRIWARE62E9D3D4(player_id, cbfunc);
                }	
			}

			internal static System.Int32 GetSubtitleOnTime(int player_id, System.IntPtr subtitle_buffer, int subtitle_buffer_size)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWARE65D61A6A)))
                {
					return CRIWARE65D61A6A(player_id, subtitle_buffer, subtitle_buffer_size);
                }
			}

/* @cond notpublic */
			internal static void SetDecryptionKey(int player_id, UInt64 key)
			{
				using (new CriDurationMeasurementScope(nameof(CRIWAREBDD4960C)))
                {
					CRIWAREBDD4960C(player_id, key);
                }	
			}
/* @endcond */
			...
			internal static extern int CRIWAREE9FC813E();
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern int CRIWARE3A6E2552();
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern int CRIWARE94E9893B(bool useAtomExPlayer, uint maxPathLength);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE99111FE9(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE3FF6C123(int player_id, System.IntPtr binder, string path);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE74F9373A(int player_id, System.IntPtr binder, int content_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWAREAC81C8CF(int player_id, string path, System.UInt64 offset, System.Int64 range);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWAREBD7E64E1(int player_id, IntPtr data, System.Int64 datasize);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWAREBD7E64E1(int player_id, byte[] data, System.Int64 datasize);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern bool CRIWARE340855B4(int player_id, System.IntPtr binder, string path, bool repeat);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern bool CRIWARE89F5F4E8(int player_id, System.IntPtr binder, int content_id, bool repeat);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern bool CRIWARE2B9092A9(int player_id, string path, System.UInt64 offset, System.Int64 range, bool repeat);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern bool CRIWARE6587A256(int player_id, IntPtr data, System.Int64 datasize, bool repeat);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern bool CRIWARE6587A256(int player_id, byte[] data, System.Int64 datasize, bool repeat);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE35201A91(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern System.Int32 CRIWARE1354FEA7(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE62E9D3D4(
				int player_id,
				CuePointCallbackFromNativeDelegate cbfunc
			);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE11EC40C0(
				int player_id,
				SubtitleCallbackFromNativeDelegate cbfunc
			);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE9A8AC346(int player_id, [Out] MovieInfo movie_info);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern int CRIWAREC2475753(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE7652287A(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE439340B3(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWAREFC4BD812(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWAREAA815027(int player_id, int seek_frame_no);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARED3E36645(int player_id, MovieEventSyncMode mode);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE2AC4CA7E(int player_id, int sw);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern bool CRIWARE88ED0C7C(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE284D752E(int player_id, int sw);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWAREE054EF2C(int player_id, bool flag);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern long CRIWARE626DF59A(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern int CRIWARE015F102F(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern IntPtr CRIWARE4E36FD70(int player_id, uint track_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern int CRIWAREFD3EF64A(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE0CA1216B(int player_id, int track);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE4F06C1D4(int player_id, float vol);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern float CRIWARE086C6A97(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE9EC619B6(int player_id, int track);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE9B69D97C(int player_id, float vol);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern float CRIWAREE218A3A4(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE894B55A2(int player_id, int track);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE27EEE694(int player_id, float vol);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern float CRIWAREF299792C(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARED5BBE158(int player_id, string bus_name, float level);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE068E5808(int player_id, string bus_name, float level);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE5604C40B(int player_id, string bus_name, float level);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE6F6D87FE(int player_id, int channel);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern System.Int32 CRIWARE65D61A6A(int player_id, System.IntPtr subtitle_buffer, int subtitle_buffer_size);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE61B8FA6A(int player_id, float speed);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWAREECCF5E62(int player_id, System.UInt32 max_data_size);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWAREB73990A9(int player_id, float sec);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWAREC4A02CCE(int player_id, int min_buffer_size);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE35F190C7(int player_id, int asr_rack_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWAREF0F20CEB(int player_id, float quality);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE74BAE3A5(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE8FCF2B74(int player_id, TimerType timer_type);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE77CA0811(int player_id, ulong user_count, ulong user_unit);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE903CEF6A(int player_id, ulong timer_unit_n, ulong timer_unit_d);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE4973B306(int player_id);
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWARE041A49E0(int player_id);
/* @cond notpublic */
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
			internal static extern void CRIWAREBDD4960C(int player_id, UInt64 key);
/* @endcond */
#if !CRIPLUGIN_USE_OLD_LOWLEVEL_INTERFACE
			[DllImport(CriWare.Common.pluginName, CallingConvention = CriWare.Common.pluginCallingConvention)]
...
#endif
#else
			internal Status _dummyNativeStatus = Status.Error;
			internal bool _dummyPaused = false;
			internal bool _dummyLoop = false;
			internal static int CRIWAREE9FC813E() { return 0; }
			internal static int CRIWARE3A6E2552() { return 0; }
			internal static int CRIWARE94E9893B(bool useAtomExPlayer, uint maxPathLength) { return 0; }
			internal static void CRIWARE99111FE9(int player_id) { }
			internal static void CRIWARE3FF6C123(int player_id, System.IntPtr binder, string path) { }
			internal static void CRIWARE74F9373A(int player_id, System.IntPtr binder, int content_id) { }
			internal static void CRIWAREAC81C8CF(int player_id, string path, System.UInt64 offset, System.Int64 range) { }
			internal static void CRIWAREBD7E64E1(int player_id, IntPtr data, System.Int64 datasize) { }
			internal static void CRIWAREBD7E64E1(int player_id, byte[] data, System.Int64 datasize) { }
			internal static bool CRIWARE340855B4(int player_id, System.IntPtr binder, string path, bool repeat) { return true; }
			internal static bool CRIWARE89F5F4E8(int player_id, System.IntPtr binder, int content_id, bool repeat) { return true; }
			internal static bool CRIWARE2B9092A9(int player_id, string path, System.UInt64 offset, System.Int64 range, bool repeat) { return true; }
			internal static bool CRIWARE6587A256(int player_id, IntPtr data, System.Int64 datasize, bool repeat) { return true; }
			internal static bool CRIWARE6587A256(int player_id, byte[] data, System.Int64 datasize, bool repeat) { return true; }
			internal static void CRIWARE35201A91(int player_id) { }
			internal static System.Int32 CRIWARE1354FEA7(int player_id) { return 0; }
			internal static void CRIWARE62E9D3D4(int player_id, CuePointCallbackFromNativeDelegate cbfunc) { }
			internal static void CRIWARE9A8AC346(int player_id, [Out] MovieInfo movie_info) { movie_info = new MovieInfo(); }
			internal static int CRIWAREC2475753(int player_id ) { return 0; }
			internal void CRIWARE7652287A(int player_id) { _dummyPaused = false; _nativeStatus = Status.WaitPrep; }
			internal void CRIWARE439340B3(int player_id) { _dummyPaused = false; _nativeStatus = Status.WaitPrep; }
			internal void CRIWAREFC4BD812(int player_id) { _nativeStatus = Status.Stop;  }
			internal static void CRIWAREAA815027(int player_id, int seek_frame_no) { }
			internal static void CRIWARED3E36645(int player_id, MovieEventSyncMode mode){}
			internal void CRIWARE2AC4CA7E(int player_id, int sw) { _dummyPaused = (sw == 1); }
			internal static bool CRIWARE88ED0C7C(int player_id) { return false; }
			internal void CRIWARE284D752E(int player_id, int sw) { _dummyLoop = (sw == 1); }
			internal static void CRIWAREE054EF2C(int player_id, bool flag) { }
			internal static long CRIWARE626DF59A(int player_id) { return 0; }
			internal static int CRIWARE015F102F(int player_id) { return 0; }
			internal static IntPtr CRIWARE4E36FD70(int player_id, uint track_id) { return new IntPtr(1); }
			internal static int CRIWAREFD3EF64A(int player_id) { return -1; }
			internal static void CRIWARE0CA1216B(int player_id, int track) { }
			internal static void CRIWARE4F06C1D4(int player_id, float vol) { }
			internal static float CRIWARE086C6A97(int player_id){ return 0f; }
			internal static void CRIWARE9EC619B6(int player_id, int track) { }
			internal static void CRIWARE9B69D97C(int player_id, float vol) { }
			internal static float CRIWAREE218A3A4(int player_id){ return 0f; }
			internal static void CRIWARE894B55A2(int player_id, int track) { }
			internal static void CRIWARE27EEE694(int player_id, float vol) { }
			internal static float CRIWAREF299792C(int player_id){ return 0f; }
			internal static void CRIWARED5BBE158(int player_id, string bus_name, float level) { }
			internal static void CRIWARE068E5808(int player_id, string bus_name, float level) { }
			internal static void CRIWARE5604C40B(int player_id, string bus_name, float level) { }
			internal static void CRIWARE6F6D87FE(int player_id, int channel) { }
			internal static void CRIWARE11EC40C0(int player_id, SubtitleCallbackFromNativeDelegate cbfunc) { }
			internal static System.Int32 CRIWARE65D61A6A(int player_id, System.IntPtr subtitle_buffer, int subtitle_buffer_size) { return 0; }
			internal static void CRIWARE61B8FA6A(int player_id, float speed) { }
			internal static void CRIWAREECCF5E62(int player_id, System.UInt32 max_data_size) { }
			internal static void CRIWAREB73990A9(int player_id, float sec) { }
			internal static void CRIWAREC4A02CCE(int player_id, int min_buffer_size) { }
			internal static void CRIWARE35F190C7(int player_id, int asr_rack_id) { }
			internal static void CRIWAREF0F20CEB(int player_id, float quality) { }
			internal static void CRIWARE74BAE3A5(int player_id) { }
			internal static void CRIWARE8FCF2B74(int player_id, TimerType timer_type) { }
			internal static void CRIWARE77CA0811(int player_id, ulong user_count, ulong user_unit) { }
			internal static void CRIWARE903CEF6A(int player_id, ulong timer_unit_n, ulong timer_unit_d) { }
			internal static void CRIWARE4973B306(int player_id) { }
			internal static void CRIWARE041A49E0(int player_id) { }
/* @cond notpublic */
			internal static void CRIWAREBDD4960C(int player_id, UInt64 key){}
/* @endcond */
#endif
        }

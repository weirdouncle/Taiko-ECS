...
namespace CriTimeline.Mana
{
	...
	public class CriManaMixerBehaviour : PlayableBehaviour {
		...
		private bool PrepareMovie(CriManaClipBase clipAsset) {
			...
			m_boundMovieMaterial.player.PrepareForRendering();
		...
		public override void ProcessFrame(Playable playable, FrameData info, object playerData) {
			...
			// making sure the pause state is correct on the current frame
			if (m_PlayableDirector.state == UnityEngine.Playables.PlayState.Paused) {
				PausePlayer(true);
			} else {
				if (clipState == ClipState.Play) {
					PausePlayer(false);
				}
			}
		...
		private void PausePlayer(bool pause) {
			...
			if (m_boundMovieMaterial.player.IsPaused() == pause) { return; }
		...
		public override void OnPlayableCreate(Playable playable) {
			...
			if (IsEditMode) {
				...
				if (CriManaPlugin.IsLibraryInitialized() == false) {
					CriWareInitializer cfg = null;
#if UNITY_2023_1_OR_NEWER
					cfg = GameObject.FindAnyObjectByType<CriWareInitializer>();
#else
					cfg = GameObject.FindObjectOfType<CriWareInitializer>();
#endif
					if (cfg != null) {

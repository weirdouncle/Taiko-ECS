...
public class LoaderScript : MonoBehaviour
{
    ...
    public GameObject[] RollingNote_pres;
    ...
    protected virtual void Decode(string str)
    {
        ...
        #region [ 删除#END之后的 ]
        float end_time = 10000000;
        foreach (CChip chip in Others)
        {
            if (chip.Type == CChip.CType.END)
            {
                end_time = chip.JudgeTime;
                break;
            }
        }
        ...
        */
        ...
        foreach (NoteChip chip in p1_notes)
        {
            ...
            switch (chip.Type)
            {
                case 1:
                case 21:
                    sound = Instantiate(NormalDon_pre, parent);
                    break;
                case 2:
                case 22:
                    sound = Instantiate(NormalKa_pre, parent);
                    break;
                case 3:
                case 10:
                    sound = Instantiate(BigDon_pre, parent);
                    break;
                case 4:
                case 11:
                    sound = Instantiate(BigKa_pre, parent);
                    break;
                case 5:
                    {
                        sound = Instantiate(Rapid_pre, parent);
                ...
                case 6:
                    {
                        sound = Instantiate(BigRapid_pre, parent);
                        RapidScript rapid = sound.GetComponent<RapidScript>();
                        rapid.EndTime = chip.EndTime;
                    }
                    break;
                case 7:
                    {
                        sound = Instantiate(Ballon_pre, parent);
                ...
                case 9:
                    {
                        ...
                        {
                            kusu = true;
                            sound = Instantiate(Hammer_pre, parent);
                            BallonScript balloon = sound.GetComponent<BallonScript>();
                            balloon.HitCount = chip.HitCount;
                            balloon.EndTime = chip.EndTime;
                        ...
                        {
                            kusu = true;
                            sound = Instantiate(Kusudama_pre, parent);
                            BallonScript balloon = sound.GetComponent<BallonScript>();
                            balloon.HitCount = chip.HitCount;
                            balloon.EndTime = chip.EndTime;
                        }
                ...
                case 20:
                    rolling_note = true;
                    sound = Instantiate(RollingNote_pres[chip.Type == 19 ? 0 : 1], parent);
                    break;
            }
            ...
            script.Adjust = GameSetting.Config.NoteAdjust;
            script.Replay = !ai_battle && GameSetting.Mode == CommonClass.PlayMode.Replay;
            if (!ai_battle)
                script.Player2 = GameSetting.Mode == CommonClass.PlayMode.PlayWithReplay && GameSetting.Config.PlayRight;

            script.Type = (chip.Type != 9 || (!ai_battle && (Score)GameSetting.Config.ScoreMode != Score.Shin)) ? chip.Type : 7;
            if (chip.Type == 10 || chip.Type == 11) script.Type = (chip.Type == 10 ? 3 : 4);
            if (chip.Type == 21 || chip.Type == 22)
            {
                script.Type = (chip.Type == 21 ? 1 : 2);
                script.Note_State = NoteSoundScript.NoteState.Stop;
            }
    ...
    private static readonly Regex regexForStrippingHeadingLines = new Regex(
        @"^(?!(LEVEL|BPM|OFFSET|BALLOON|BALLOONNOR|BALLOONEXP|BALLOONMAS|SCOREINIT|SCOREDIFF|COURSE|STYLE|GAME|LIFE|SIDE|SCOREMODE|MOVIEOFFSET|BGIMAGE|BGMOVIE|HIDDENBRANCH|GAUGEINCR|#HBSCROLL|#BMSCROLL|#ABPM)).+\n",
    ...
    private void t入力_行解析ヘッダ(string InputText)
    {
        ...
        {
            float abpm = -1;
            bool abpm_enable = false;
            if (strCommandParam.IndexOf(",") != -1) strCommandParam = strCommandParam.Replace(',', '.');
            if (strCommandParam.IndexOf("@") != -1)
            {
                string[] paras = strCommandParam.Split(new char[] { '@' });

                strCommandParam = paras[0].Trim();
                string AbpmParam = paras[1].Trim();
                abpm_enable = float.TryParse(AbpmParam, out abpm);
            }

            float dbBPM = (float)Convert.ToDouble(strCommandParam);
            if (this.BPM <= 0) this.BPM = dbBPM;
            this.dbNowBPM = dbBPM;

            if (!abpm_enable) abpm = dbBPM;
    ...
    private string[] tコマンド行を削除したTJAを返す(string[] input, int nMode)
    {
        ...
        for (int n = 0; n < input.Length; n++)
        {
            ...
            else if (nMode == 1)
            {
                ...
                {
                    if (input[n].StartsWith("BALLOON") || input[n].StartsWith("BPM") || input[n].StartsWith("LEVEL"))
                    {
                                ...
                                if (sound != null)
                                {
                                    ...
                                    for (int index = Others.Count - 1; index >= 0; index--)
                                    {
                                        ...
                                        if (chip.Type == CChip.CType.GOGO_TIME)
                                        {
                                            ...
                                            {
                                                if (sound.Chapter <= chip.EndChapter || chip.EndChapter == 0)
                                                {
                                                    sound.Gogo = true;
                                                }
                                            }
                                    ...
                                    if (ListBRANCH.Count > 0)
                                    {
                                        CBRANCH branch = ListBRANCH[ListBRANCH.Count - 1];
                                        if (sound.Chapter >= branch.StartChapter && (branch.EndChapter == 0 || sound.Chapter < branch.EndChapter))
    ...
    private void InsertCommand(string InputText)
    {
        ...
        else if (command == "#END")
        {
            ...
            if (ListBRANCH.Count > 0 && ListBRANCH[ListBRANCH.Count - 1].EndChapter == 0)
                ListBRANCH[ListBRANCH.Count - 1].EndChapter = current_chapter_index;

            //Debug.Log(chip.JudgeTime);
            Others.Add(chip);
        ...
        /*
        else if (command == "#HBPMCHANGE")
        {
            Debug.Log("h bpm change");
            float dbBPM = (float)Convert.ToDouble(argument);
            this.dbNowBPM = dbBPM;
        ...
        */
        else if (command == "#ABPM")
        {
            float dbBPM = (float)Convert.ToDouble(argument);
            CChip chip = new CChip
            {
                Type = CChip.CType.ANIMATION_BPM_CHANGE,
                JudgeTime = (int)this.dbNowTime + this.nOFFSET,
                Bpm = dbBPM,
                StartChapter = current_chapter_index
            };
        ...
        else if (command == "#GOGOEND")
        {
            for (int i = Others.Count - 1; i >= 0; i--)
            {
                ...
                {
                    Others[i].EndChapter = current_chapter_index;
        ...
        else if (command == "#BRANCHSTART")
        {
            ...
            */
                    ...
                    foreach (CChip _chip in Others)
                    {
                        if (_chip.Type == CChip.CType.BRANCH_NOTICE || _chip.Type == CChip.CType.BRANCH_START)
                        {
                            if (_chip.BranchIndex == this.n内部番号BRANCH1to - 1)
                                _chip.EndChapter = current_chapter_index;
                        }
                    }
                }
            }
            ...
            {
                CChip old = Others[i];
                if (old.StartChapter < from_chapter && (old.Type == CChip.CType.BRANCH_RESET || old.Type == CChip.CType.BRANCH_NOTICE))
                {
...
#if UNITY_EDITOR
            Debug.Log(string.Format("branch index count {0} from {1} to {2}", n内部番号BRANCH1to, from_chapter, to_chapter));
#endif
            CChip chip = new CChip
            {
            ...
            int branch_time = (int)this.dbNowTime + this.nOFFSET;
#if UNITY_EDITOR
            ...
            CChip chip2 = new CChip
            {
                ...
                BranchIndex = this.n内部番号BRANCH1to,
                StartChapter = to_chapter,
                EndChapter = from_chapter
            };
            Others.Add(chip2);
            ...
            {
                BranchNoticeTime = chip.JudgeTime,
                ...
                StartChapter = this.current_chapter_index,
                n条件数値A = (float)nNum[0],
                n条件数値B = (float)nNum[1],
        ...
        else if (command == "#N")
        {
            ...
            this.current_chapter_index = branch.StartChapter;
        ...
        else if (command == "#BRANCHEND")
        {
            ...
            foreach (CChip chip in Others)
            {
                if (chip.Type == CChip.CType.BRANCH_NOTICE) 
                {
                    if (chip.BranchIndex == this.n内部番号BRANCH1to - 1)
                        chip.EndChapter = current_chapter_index;
        ...
        else if (command == "#BARLINEON")
        {
            for (int i = Others.Count - 1; i > 0; i--)
            {
                ...
                {
                    chip1.EndChapter = current_chapter_index;
                    break;
                }
            }
